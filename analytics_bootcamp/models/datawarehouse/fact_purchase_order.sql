...
with source as(
    ...
    from {{ ref('staging_purchase_orders') }} po
    left join {{ ref('staging_purchase_order_details') }} pod
    on pod.purchase_order_id = po.id
    left join {{ ref('staging_products') }} p
    on p.id = pod.product_id
    left join {{ ref('staging_order_details') }} od
    on od.product_id = p.id
    left join {{ ref('staging_orders') }} o
    on o.id = od.order_id
    left join {{ ref('staging_employee') }} e
    on e.id = po.created_by
    left join {{ ref('staging_customer') }} c
